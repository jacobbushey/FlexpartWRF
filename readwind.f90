...
!   (interpolate it from "U-grid" to "T-grid" later)
      ...
      ! Variation of U wind component for error analysis (JK, 11.06.2019)
      !write(*,*) uuh(128,80,kbgn+2)
!      do m=0,lendim(3)
!      do j=0,lendim(2)
!      do i=0,lendim(1)
!        uuh(i,j,kbgn+m) = uuh(i,j,kbgn+m) + sign(0.686, uuh(i,j,kbgn+m))
!      enddo
!      enddo
!      enddo
      !write(*,*) uuh(128,80,kbgn+2)
...
!   (interpolate it from "V-grid" to "T-grid" later)
      ...
      ! Variation of V wind component for error analysis (JK, 11.06.2019)
      !write(*,*) vvh(128,80,kbgn+2)
!      do m=0,lendim(3)
!      do j=0,lendim(2)
!      do i=0,lendim(1)
!        vvh(i,j,kbgn+m) = vvh(i,j,kbgn+m) + sign(0.686, vvh(i,j,kbgn+m))
!      enddo
!      enddo
!      enddo
      !write(*,*) vvh(128,80,kbgn+2)
!      write(*,9100) 'Add 0.686m/s on U and V'
      ...
      ! Variation of U and V wind component for error analysis (JK, 11.06.2019)
!      do m=0,lendim(3)
!      do j=0,lendim(2)
!      do i=0,lendim(1)
!        uuh(i,j,kbgn+m) = uuh(i,j,kbgn+m)*0.996194698091746 - vvh(i,j,kbgn+m)*0.087155742747658
!        vvh(i,j,kbgn+m) = vvh(i,j,kbgn+m)*0.996194698091746 + uuh(i,j,kbgn+m)*0.087155742747658
!      enddo
!      enddo
!      enddo
!      write(*,9100) 'Add 5 degrees on U and V'
...
! pblh
      ...
      call read_ncwrfout_1realfield( ierr, idiagaa, fnamenc, &
          ...
          ndims, ndims_exp, ndims_max, &
          lendim, lendim_exp, lendim_max )
!      write(*,*) 'Using PBLH from WRF ', lendim
!      write(*,*) hmix(128,80,1,n);
      do j=0,lendim(2)
      do i=0,lendim(1)
        hmix(i,j,1,n) = hmix(i,j,1,n) + 100;
      enddo
      enddo
!      write(*,*) hmix(128,80,1,n); (JK, 27.3.2020) debug output PBLH
      if (ierr .ne. 0) then

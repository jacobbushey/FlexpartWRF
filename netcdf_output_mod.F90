...
10 format('nc_create_header_outfile: Setting up ',A)
  ...
  !*****************************************************************************
    ...
    integer :: jkix,jkjy
    real :: jkhmix,jkhmixsd

    real :: xl,yl,xl2,yl2
    integer :: iix,jjy
      
    if (option_verbose.ge.50) then
...
!      do jkjy=0,nymin1
!        do jkix=0,nxmin1
!          jkhmix = jkhmix + hmix(jkix, jkjy, 1, memind(2));
!        enddo
!      enddo
!      write(*,*) 'pblh:', jkhmix/(nymin1*nxmin1)
...
!              
!            grid2(ix,jy,kz,kp,nage)= grid(ix,jy,kz)*1.e12/(area(ix,jy)*hmix(iix,jjy,1,memind(2)))/outnum/tot_mu_scalar
    ...
    if ((iout.eq.2).or.(iout.eq.3)) then  ! mixing ratio
      ...
      ! JK 2023: trying to get mean mixing ratio in PBL
!      jkhmix=0
!      do jkjy=0,nymin1
!        do jkix=0,nxmin1
!          jkhmix = max(jkhmix, hmix(jkix, jkjy, 1, memind(2)))
!        enddo
!      enddo
      ...
      do kz=1,numzgrid
        do jy=0,grid_ny-1
          do ix=0,grid_nx-1
            ...
            ! ######################################################################
            ! ######################################################################
            !write(*,*) 'Area: ', ix, jy, area(ix,jy), jkhmix
...
!              jjy=max(min(nint(yl),nymin1),0)
!
!              !write(*,*) 'Area: ', ix, jy, area(ix,jy), jkhmix, hmix(iix,jjy,1,2)
!            grid3(ix,jy,kz,kp,nage)= 1.e12*grid(ix,jy,kz)/(area(ix,jy)*hmix(iix,jjy,1,2))/outnum*  &
!              weightair/weightmolar(ks)/densityoutgrid(ix,jy,kz)

            !grid3(ix,jy,kz,kp,nage)= 1.e12*grid(ix,jy,kz)/grid_vol(ix,jy,kz)/outnum*  &
            !  weightair/weightmolar(ks)/densityoutgrid(ix,jy,kz)
            
              grid3(ix,jy,kz,kp,nage)= 1.e12*grid(ix,jy,kz)/outnum/weightmolar(ks)            !n_species
              grid4(ix,jy,kz,kp,nage)= grid_vol(ix,jy,kz)*densityoutgrid(ix,jy,kz)/weightair  !n_air

          enddo ! ix=1,grid_nx-1
        enddo ! jy=1,grid_ny-1
      enddo ! kz=1,numzgrid

      ! Iterate over z-levels and collapse to boundary layer enhancement in grid3 : layer 1
      do kz=2,numzgrid
        do jy=0,grid_ny-1
          do ix=0,grid_nx-1
            
            ! need to add the last part between zero concentration and hmix
            if (outheight(kz).lt.pblhoutgrid(ix,jy)) then
              grid3(ix,jy,1,kp,nage)= grid3(ix,jy,1,kp,nage) + grid3(ix,jy,kz,kp,nage)
              grid4(ix,jy,1,kp,nage)= grid4(ix,jy,1,kp,nage) + grid4(ix,jy,kz,kp,nage)
            else
              !write(*,*) 'outheight below pbl', outheight(kz), pblhoutgrid(ix,jy)
              if (outheight(kz-1).lt.pblhoutgrid(ix,jy)) then
              ! add moles of air between last level below and PBL (+100m transition zone)
              grid4(ix,jy,1,kp,nage)= grid4(ix,jy,1,kp,nage) + &
                area(ix,jy)*(pblhoutgrid(ix,jy)-outheight(kz-1)+100)*densityoutgrid(ix,jy,kz-1)/weightair
              !write(*,*) 'add density at pbl', outheight(kz), pblhoutgrid(ix,jy)
              !difftopblgrid(ix,jy)= pblhoutgrid(ix,jy)-outheight(kz-1)
              endif
            endif
      ...
      do jy=0,grid_ny-1
        do ix=0,grid_nx-1
            grid3(ix,jy,1,kp,nage)= grid3(ix,jy,1,kp,nage) / grid4(ix,jy,1,kp,nage)
            grid3(ix,jy,2,kp,nage)= pblhoutgrid(ix,jy)
            grid3(ix,jy,3,kp,nage)= densityoutgrid(ix,jy,1)
            grid3(ix,jy,4,kp,nage)= grid4(ix,jy,1,kp,nage)
            !grid3(ix,jy,5,kp,nage)= densityoutgrid(ix,jy,numzgrid+1)
            !grid3(ix,jy,6,kp,nage)= difftopblgrid(ix,jy)
        enddo ! ix=1,grid_nx-1
      enddo ! jy=1,grid_ny-1

...
!*******************************************************************
  ...
  do jy=0,numygrid-1
    do ix=0,numxgrid-1
      ...
      pblhoutgrid(ix,jy)=hmix(iix,jjy,1,memind(2))
    end do
  end do
...
!      write(*,*) 'Using PBLH from WRF ', hmix(128,80,1,2)
    else
...
!        yl=(yl-ymet0)/dx
  ...
  ! get density at PBL height
  ! ******************************************************************
!    do jy=0,numygrid-1
!      do ix=0,numxgrid-1
!        
!        ! get nearest neighbors to PBL in output grid
!        do kzz=2,nz
!        if ((height(kzz-1).lt.pblhoutgrid(ix,jy)).and. &
!           (height(kzz).gt.pblhoutgrid(ix,jy))) goto 47
!        end do
!47      kzz=max(min(kzz,nz),2)
!        dz1=pblhoutgrid(ix,jy)-height(kzz-1)
!        dz2=height(kzz)-pblhoutgrid(ix,jy)
!        dz=dz1+dz2
!
!        xl2=outlon0+float(ix)*dxoutl !long
!        yl2=outlat0+float(jy)*dyoutl !lat
...
!        jjy=max(min(nint(yl),nymin1),0)
!
!        densityoutgrid(ix,jy,numzgrid+1)=(rho(iix,jjy,kzz,2)*dz1+ &
!             rho(iix,jjy,kzz-1,2)*dz2)/dz
!      end do
!    end do
